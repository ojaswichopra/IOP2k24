...
def render_feedback():
    st.markdown("<h2 style='text-align: center; color: #2B8C0C'>Feedback</h2>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center'>Please provide your feedback or report any issues encountered while using the application.</p>", unsafe_allow_html=True)
    ...
    if st.button("Submit"):
        # Placeholder for feedback submission logic (e.g., send feedback to database, email)
...
def main():
    ...
    # Images Section
    st.image('../assets/Landing.jpg', caption='TinyTot Diagnosis: Empowering Care, Transforming Lives')

    # Story Section
    st.markdown("<h2 style='text-align: center; color: #2B8C0C'>Our Story</h2>",unsafe_allow_html=True)
    st.markdown("<p>Every child deserves a healthy and happy start in life. Inspired by this belief, we embarked on a journey to harness the power of artificial intelligence for the benefit of pediatric healthcare. Our interdisciplinary team of data scientists, pediatricians, and caregivers came together with a shared vision: to create a world where every child\'s health is safeguarded from the very beginning.</p>", unsafe_allow_html=True)
    st.write("---")
    
    # Power of Prediction
    st.markdown("<h2 style='text-align: center; color: #2B8C0C'>The Power of Prediction</h2>",unsafe_allow_html=True)
    st.markdown("<p>Imagine being able to anticipate health challenges before they even manifest. With our predictive models, we\'re turning this vision into reality. From childhood anxiety disorder to pneumonia, jaundice to asthma, cerebral palsy to Down syndrome and autism disorder, our models analyze a wealth of data to identify potential risks early on..</p>", unsafe_allow_html=True)
    st.write("---")

    # Insights Section
    st.markdown("<h2 style='text-align: center; color: #2B8C0C'>Insights That Matter</h2>",unsafe_allow_html=True)
    st.markdown("<p>It\'s not just about prediction—it\'s about understanding. Our models don\'t just flag risks; they provide deep insights into each condition, helping caregivers make informed decisions about treatment and management. Whether it\'s uncovering patterns in symptom progression, identifying genetic markers, or recommending personalized interventions, TinyTot Diagnosis is dedicated to equipping caregivers with the knowledge they need to provide the best possible care for every child.</p>", unsafe_allow_html=True)
    st.write("---")
    ...
    st.markdown("<p>At the heart of TinyTot Diagnosis is a commitment to empowerment. We believe that by arming caregivers with powerful tools and knowledge, we can make a tangible difference in the lives of children and families around the world. Our platform is designed to be intuitive, accessible, and user-friendly, ensuring that caregivers can seamlessly integrate our insights into their daily practice, making informed decisions that impact lives for the better.</p>", unsafe_allow_html=True)
    st.write("---")
    
    # Join Us Section
    st.markdown("<h2 style='text-align: center; color: #2B8C0C'>Join Us in Shaping the Future</h2>",unsafe_allow_html=True)
    st.markdown("<p>The journey towards a healthier future for children begins with you. Whether you\'re a caregiver, a healthcare professional, or simply someone passionate about pediatric health, we invite you to join us in our mission. Explore our platform, engage with our community, and together, let\'s shape a world where every child has the opportunity to thrive.</p>", unsafe_allow_html=True)
    st.write("---")
